...
namespace revattr
{
	...
	function attr_extra_19_crimson(&$pa,&$pd,$active,$phase=0)
	{
		...
		# 真红暮作为防御方时的事件
		if($pd['type'] == 19 && $pd['name'] == '紅暮' && $pa['wep_kind'] != $pd['wep_kind'] && $phase == 'defend')
		{	
			$log .= "<span class=\"red\">紅暮身上的武器投射出了防護罩，輕鬆擋下了{$pa['nm']}的攻擊！</span><br>";
			return 0;
		}

		# 真红暮作为进攻方时的事件：
		if($pa['type'] == 19 && $pa['name'] == '紅暮' && $phase == 'attack')
		{
			$log .= "<span class=\"yellow\">“那麼説好了，不留手咯~”<br></span>";
			//$log .= "红暮吐气扬声，向你袭来！<br>";
	...
	function attr_extra_89_efox(&$pa,&$pd,$active,$phase=0)
	{
		...
		if ($pa['type'] == 89 && $pa['name'] == '電掣部長 米娜')
		{
			...
			elseif($dice<=635)
			{
				...
				<span class=\"neonblue\">“你的<span class=\"red\">{$pd['wep']}</span>，已掃描入<span class=\"red\">$dice2</span>號位。”<br>
				“我會妥善保管的~”</span><br>";
		...
		if($pd['type'] == 89)
		{
			...
			//Adding an additional check to reset a negative rp value to 1 before other rp checks to deal with an oddity.
			{
				if($phase == 'rp')
				{
					...
					if ($pa['rp'] < 0){
						$log .= "<span class=\"glitch1\">“不管你之前積了多少德，一個決定就是一個決定。”</span><br>";
						$pa['rp'] = 1;	
					}
					$dice = diceroll(444);
					if($dice<=200){
	...
	function attr_extra_89_walksheep(&$pa,&$pd,$active,$phase=0)
	{
		...
		if($pa['type'] == 89 && $pa['name'] == '堅韌之子·拉姆')
		{
			...
			if($event_dice >=30)
			{
				...
				{
					$log .= "攻擊使得<span class=\"red\">{$pd['wep']}</span>的效果下降了<span class=\"red\">$damage</span>點！<br>";
	...
	function attr_extra_89_minibee(&$pa,&$pd,$active,$phase=0)
	{
		...
		if ($pa['type'] == 89 && $pa['name'] == '誠心使魔·阿摩爾') // 迷你蜂
		{ 
	...
	{
		if(!empty($pa['arbs']) && $pa['arb'] == '【智代專用熊裝】')
		{
			// 必须连续攻击同一个对象
